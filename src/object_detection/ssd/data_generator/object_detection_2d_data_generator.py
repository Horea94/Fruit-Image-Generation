...
class DataGenerator:
    ...
    def parse_csv(self,
        ...
        if self.annotations_dir is None or self.input_format is None:
            raise ValueError("`annotations_dir` and/or `input_format` have not been set yet. You need to pass them as arguments.")
        ...
        data = []
        for annot_file in os.listdir(self.annotations_dir):
            annot_file_path = os.path.join(self.annotations_dir, annot_file)
            with open(annot_file_path) as csvfile:
                lines = [k.strip() for k in csvfile.readlines()]
                img_name = lines[0]
                lines = lines[1:]
                for line in lines:  # For every line (i.e for every bounding box) in the CSV file...
                    row = line.split(',')
                    if self.include_classes == 'all' or int(row[self.input_format.index('class_id')].strip()) in self.include_classes:  # If the class_id is among the classes that are to be included in the dataset...
                        ...
                        box.append(img_name)  # Select the image name column in the input format and append its content to `box`
                        for element in self.labels_output_format:  # For each element in the output format (where the elements are the class ID and the four box coordinates)...
                            if element == 'class_id':
                                box.append(all_labels.index(row[self.input_format.index(element)]))
                            else:
