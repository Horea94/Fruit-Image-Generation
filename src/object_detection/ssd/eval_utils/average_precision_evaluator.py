...
from ssd.data_generator.object_detection_2d_patch_sampling_ops import RandomPadFixedAR
from ssd.data_generator.object_detection_2d_photometric_ops import ConvertTo3Channels
from ssd.ssd_encoder_decoder.ssd_output_decoder import decode_detections
...
class Evaluator:
    ...
    def match_predictions(self,
        ...
        for class_id in range(1, self.n_classes + 1):
            ...
            num_chars_per_image_id = 0
            for pred in predictions:
                if len(str(pred[0])) > num_chars_per_image_id:
                    num_chars_per_image_id = len(str(pred[0]))
