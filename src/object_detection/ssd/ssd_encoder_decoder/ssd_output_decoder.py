...
from ssd.bounding_box_utils.bounding_box_utils import iou, convert_coordinates
...
def decode_detections(y_pred,
    ...
    y_pred_decoded_raw = np.copy(
        y_pred[:, :, :-8])  # Slice out the classes and the four offsets, throw away the anchor coordinates and variances, resulting in a tensor of shape `[batch, n_boxes, n_classes + 4 coordinates]`
...
def get_pred_layers(y_pred_decoded: np.ndarray, num_boxes_per_pred_layer):
    """
